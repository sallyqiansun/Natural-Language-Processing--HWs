...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        if len(state.stack)!=0:            
            if len(state.stack)==1:
                temp[0]=state.stack[-1]
            elif len(state.stack)==2:
                temp[0]=state.stack[-2]
                temp[1]=state.stack[-1]
            else:
                temp[0]=state.stack[-3]
                temp[1]=state.stack[-2]
                temp[2]=state.stack[-1]
            
        temp2=[None]*3
        if len(state.buffer)!=0:
            if len(state.buffer)==1:
                temp2[0]=state.buffer[-1]
            elif len(state.buffer)==2:
                temp2[0]=state.buffer[-2]
                temp2[1]=state.buffer[-1]
            else:
                temp2[0]=state.buffer[-3]
                temp2[1]=state.buffer[-2]
                temp2[2]=state.buffer[-1]
        temp.extend(temp2)
        ...
        for i in range(len(temp)):
            ...
            if index is None:
                ...
                out[i]=1
            elif words[index] not in self.word_vocab.keys():
                out[i]=2
            elif index==0:
                out[i]=3
            else:
                out[i]=self.word_vocab[words[index]]
        return out
    ...
    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
        out=np.zeros(91)
        if output_pair in self.output_labels.keys():
            out[self.output_labels[output_pair]]=1
        return out
